...
function GalleryScene({
	...
	const [scrollVelocity, setScrollVelocity] = useState(1.0);
	...
	// Auto-play logic with speed tailored for fast 5-second intro reveal
	useEffect(() => {
	...
	useFrame((state, delta) => {
		// Apply auto-play (smooth, controlled speed for 5-second intro)
		if (autoPlay) {
		...
		// 8.1 depth units/sec across 40.5 units visible span (0.05 to 0.95) = exactly 5.0 seconds per image lifespan
		const zSpeed = autoPlay ? 8.1 : scrollVelocity * 8.1;

		planesData.current.forEach((plane, i) => {
			let newZ = plane.z + zSpeed * delta;
			let wrapsForward = 0;
			let wrapsBackward = 0;
...
function FallbackGallery({ images }: { images: ImageItem[] }) {
	...
	return (
		<div className="flex flex-col items-center justify-center h-full bg-[#0a2e2a] p-4 text-[#f5f1e8]">
			<p className="text-[#d4af7a] mb-4 font-semibold">
				Imóveis disponíveis no site:
			</p>
			<div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-h-96 overflow-y-auto w-full max-w-4xl">
				{normalizedImages.map((img, i) => (
					<img
						key={i}
						src={img.src || '/placeholder.svg'}
						alt={img.alt || `Imóvel ${i + 1}`}
						className="w-full h-36 object-cover rounded-xl border border-[#d4af7a]/30"
					/>
				))}
			</div>
		</div>
